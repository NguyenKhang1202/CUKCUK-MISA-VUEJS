...
class ComboboxJS {
    ...
    buildComboboxHTML() {
            ...
            for (const comboboxOrigil of mcomboboxs) {
                ...
                let required = $(comboboxOrigil).attr('required');
                let property = $(comboboxOrigil).attr('property');
                let propValue = $(comboboxOrigil).attr('propValue');
                let propText = $(comboboxOrigil).attr('propText');
                if(property == undefined && property == null){
                    var newComboboxHTML = $(`<div id="${id}" class="m-combobox" mcombobox required=${required} >
                    ...
                    var newComboboxHTML = $(`<div id="${id}" class="m-combobox" mcombobox property=${property} required=${required} >
                                                <input type="text" class="m-input">
                                                <button class="m-combobox-button"></button>
                                                <div class="m-combobox-data">
                                                </div>
    ...
    initEvents() {
        ...
        // mỗi khi sự kiện setValue đc kích hoạt, nó sẽ thực thi hàm dưới
        ...
        $.fn.setValue = function(value) {
            // mỗi lần 1 combobox đc dùng đến thì sẽ gọi hàm 1 lần
    ...
    comboboxButtonOnClick() {
        // dùng this để đảm bảo chỉ 1 combobox bị ảnh hưởng, this chính là button đc click
    ...
    /**
     * Khi chọn 1 item trong combobox: (hiển thị data vào input)
     * Author: NVMANH (21/03/2022)
     */
